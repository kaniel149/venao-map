...
export default function PropertyDetail() {
  ...
  if (!property) return <div className="pt-24 text-center text-slate-500">Property not found.</div>;
  ...
  return (
    <div className="pt-20 pb-10 px-4 max-w-6xl mx-auto">
      <Link to="/properties" className="text-sm text-[#D97706] hover:text-[#B45309] transition-colors mb-6 inline-flex items-center gap-1.5 font-medium">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            <span className="text-xs px-3 py-1 rounded-full bg-gray-100 text-slate-700 font-medium">{typeLabels[property.type]}</span>
          </div>

          <h1 className="text-3xl font-bold text-[#0F172A] mb-2">{property.title}</h1>
          <p className="text-slate-500 mb-4">{property.address}</p>
          <p className="text-4xl font-bold text-[#0F172A] mb-6">{formatPrice(property.price)} <span className="text-lg font-normal text-slate-500">{property.currency}</span></p>
          ...
          <h2 className="text-xl font-bold text-[#0F172A] mb-3">Description</h2>
          <p className="text-slate-700 leading-relaxed mb-8">{property.description}</p>
        ...
        <div>
          <div className="bg-gray-50 rounded-xl p-6 mb-6 border border-gray-100">
            <h3 className="text-lg font-bold text-[#0F172A] mb-4 flex items-center gap-2">
              <span className="w-8 h-8 bg-[#0F172A] text-white rounded-full flex items-center justify-center text-sm">👤</span>
              Agent
            </h3>
            <p className="font-semibold">{property.agent_name}</p>
            <p className="text-sm text-slate-600">{property.agent_phone}</p>
            <p className="text-sm text-slate-600 mb-4">{property.agent_email}</p>
      ...
      {similar.length > 0 && (
        <div className="mt-12">
          <h2 className="text-2xl font-bold text-[#0F172A] mb-6">Similar Properties</h2>
...
function Stat({ label, value, icon }: { label: string; value: string; icon?: string }) {
  return (
    <div className="bg-gray-50 rounded-xl p-4 text-center border border-gray-100 hover:border-[#D97706]/30 transition-colors">
      {icon && <p className="text-lg mb-1">{icon}</p>}
      <p className="font-bold text-[#0F172A] text-lg">{value}</p>
      <p className="text-xs text-slate-400 uppercase tracking-wide mt-0.5">{label}</p>
    </div>
  );
}
